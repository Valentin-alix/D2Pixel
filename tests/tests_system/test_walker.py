...
class TestWalkerSystem(unittest.TestCase):
    def setUp(self) -> None:
        ...
        logger = Logger("root")
        self.service = ServiceSession(logger=logger, app_signals=AppSignals())
        is_paused = Event()
        organizer = Organizer(
            window_info=window,
        ...
        object_searcher = ObjectSearcher(logger=logger, service=self.service)
        ...
        char_state = CharacterState(service=self.service, character_id="temp")
        spell_manager = SpellManager(
            grid=grid, service=self.service, character_state=char_state
        )
        ...
        job_parser = JobParser(service=self.service, logger=logger)
        hud_sys = HudSystem(
            hud=hud,
            image_manager=image_manager,
            character_state=char_state,
        ...
        user = UserService.get_current_user(self.service)
        self.core_walker_sys = CoreWalkerSystem(
            hud_sys=hud_sys,
            logger=logger,
            map_state=MapState(),
            character_state=char_state,
    ...
    def test_travel_world(self):
        self.core_walker_sys.travel_to_world(2)

    def test_travel_map(self):
        map = MapService.get_related_map(
            self.service,
            CoordinatesMapSchema(x=0, y=-37),
        )
        self.core_walker_sys.travel_to_map(
            [map],
            [],
            use_transport=False,
        )
