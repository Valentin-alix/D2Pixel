...
class CoreWalkerSystem(HudSystem, MapState):
    def on_new_map(self, pause: bool = True) -> tuple[bool, numpy.ndarray]:
        if pause:
            wait(RANGE_NEW_MAP)

        img = self.capture()
        if self.curr_map_info:
            from_map = self.curr_map_info.map
        else:
            from_map = None

        map, text = get_map(img, from_map)

        if (
            self.curr_map_info
            and self.curr_map_info.map == map
            and self.curr_map_info.zone_text == text
        ):
            return False, img

        if map.waypoint is not None:
            CharacterService.add_waypoint(self.character.id, map.waypoint.id)

        self.curr_map_info = CurrentMapInfo(map=map, img=img, zone_text=text)
        self.log_info(f"New map : {self.curr_map_info}")

        return True, img

    def get_curr_map_info(self) -> CurrentMapInfo:
        if self.curr_map_info is None:
            self.log_info("Current map info is None, getting info")
            self.on_new_map(False)
            assert self.curr_map_info is not None
        return self.curr_map_info

    def get_curr_direction(self) -> FromDirection:
        if self.curr_direction is None:
            self.log_info("Current direction is None, changing map")
            self.change_map()
            assert self.curr_direction is not None
    ...
    def init_first_move(
        ...
        if not (self.character.is_sub and use_transport):
        ...
        near_zaap: WaypointSchema | None = min(
            (
                ...
                for waypoint in WorldService.get_waypoints(
                    self.get_curr_map_info().map.world_id
                )
    ...
    def use_zaap(self, waypoint: WaypointSchema) -> numpy.ndarray:
        character_waypoints = CharacterService.get_waypoints(self.character.id)
        if waypoint not in character_waypoints:
            ...
            img = self.capture()
            if (
                self.get_position(img, ObjectConfigs.PathFinding.lotery_havre_sac)
                is None
            ):
            ...
            self.click(ZAAP_HAVRE_SAC_POSITION)
            tp_pos, _, img = self.wait_on_screen(
                ObjectConfigs.PathFinding.teleport_zaap, force=True
            )

            self.send_text(
                MapService.get_map(waypoint.map_id).sub_area.name,
            ...
            if (new_img := self.wait_for_new_map()) is None:
                ...
                img = new_img
            self.curr_direction = FromDirection.WAYPOINT
            self.building = None
        return img

    def use_zaapi(self, zaapi: ZaapiSchema) -> numpy.ndarray:
        self.go_out_building()
        position_zaapi = self.get_position(
            self.capture(),
            ObjectConfigs.PathFinding.zaapi,
            self.get_curr_map_info().map.id,
            force=True,
        )[0]
        self.click(position_zaapi)

        tp_pos, _, img = self.wait_on_screen(
            ObjectConfigs.PathFinding.teleport_zaap, force=True
        )
        self.click(zaapi.category.value)
        self.send_text(zaapi.text, pos=ZAAPI_SEARCH_POSITION)
        ...
        if new_img is None:
            ...
            img = new_img
        self.curr_direction = FromDirection.ZAAPI
        return self.close_modals(img, [ObjectConfigs.Cross.bank_inventory_right])
    ...
    def go_in_building(self, buildings: Sequence[BuildingInfo]) -> Any:
        ...
        self.building = near_building
        self.curr_direction = FromDirection.UNKNOWN
        return res

    def go_out_building(self) -> Any | None:
        if self.building is not None:
            res = retry_force_count(CharacterIsStuckException)(self.building.go_out)()
            self.building = None
            self.curr_direction = FromDirection.UNKNOWN
            return res
        return None
    ...
    def wait_for_new_map(
        ...
        img: numpy.ndarray | None = self.wait_animation(
    ...
    def change_map(self) -> numpy.ndarray:
        for map_direction in MapService.get_map_neighbors(
            ...
            if new_img is None:
                continue
            return new_img
        raise UnknowStateException(self.capture(), "cant_change_map")
    ...
    def __handle_neighbor_new_map(
        ...
        self.curr_direction = get_inverted_direction(map_direction.to_direction)
        self.log_info(f"New curr direction : {self.get_curr_direction()}")
        if map_direction.to_map_id == self.get_curr_map_info().map.id:
            ...
            if do_trust and not map_direction.was_checked:
                MapService.confirm_map_direction(
                    map_direction.id, map_direction.to_map_id
                )
            return img, False
        ...
        if do_trust:
            if not map_direction.was_checked:
                MapService.confirm_map_direction(
                    map_direction.id, self.get_curr_map_info().map.id
                )
            # else:
            #     raise UnknowStateException(self.capture(), "unexpected_map")
    ...
    def go_to_neighbor(
        ...
        for _ in range(MAX_RETRY):
            with self.hold(win32con.VK_SHIFT) if use_shift else nullcontext():
                self.click(pos_direction)
        ...
        if do_trust:
            if not map_direction.was_checked:
                if (
                    len(
                        MapService.get_map_neighbors(
                            self.get_curr_map_info().map.id, self.get_curr_direction()
                        )
                    )
                    <= 1
                ):
                ...
                MapService.delete_map_direction(map_direction.id)
            else:
                raise UnknowStateException(self.capture(), "unexpected_no_new_map")
    ...
    def travel_to_world(
        ...
        def dialog_portal_twelve() -> numpy.ndarray | None:
            self.click(PORTAL_TWELVE_TAKE_POSITION)
            wait((0.3, 0.5))
            self.click(PORTAL_TWELVE_ACCEPT_POSITION)
            wait((0.3, 0.5))
            self.click(PORTAL_TWELVE_CONFIRM_POSITION)
            return self.wait_for_new_map(is_same_world=False)

        def enter_portal_to_incar() -> numpy.ndarray | None:
            self.click(PORTAL_INCARNAM_ENTER_POSITION)
            return self.wait_for_new_map()

        def take_portal_to_incar() -> numpy.ndarray | None:
            self.click(PORTAL_INCARNAM_TAKE_POSITION)
            return self.wait_for_new_map(is_same_world=False)
    ...
    def travel_to_map(
        ...
        if available_waypoints is None:
            available_waypoints = CharacterService.get_waypoints(self.character.id)
